...
This report presents a comprehensive audit of back navigation implementation across all screens in the `mobile/app` directory of the Michicondrias project. A total of **155 files** (layouts, redirects, and screen files) were audited.

The audit identifies screens using the approved standardized back navigation components vs. those that use custom, non-standard, or ad-hoc back buttons and require homogenization.
...
The following 10 screens currently utilize custom `TouchableOpacity` buttons with raw icons (`ChevronLeft` or `ArrowLeft`), direct `router.back()`/`goBack()` calls, or lack headers entirely. These screens are candidates for homogenization:
