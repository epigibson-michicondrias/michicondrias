-- 🗄️ Supabase Setup - Dashboard Veterinario (CORREGIDO)
...
-- Verificar tipos de datos de las tablas clave
SELECT 
    table_name,
    column_name,
    data_type,
...
WHERE table_name IN ('medical_records', 'pets', 'clinics', 'veterinarians', 'appointments')
    AND column_name IN ('id', 'clinic_id', 'pet_id', 'veterinarian_id')
ORDER BY table_name, ordinal_position;

-- =====================================================
-- 🆕 PASO 2: CREAR TABLAS NUEVAS (ajustando tipos de datos)
-- =====================================================

-- Tabla 1: medical_records_extended (CORREGIDA)
CREATE TABLE IF NOT EXISTS medical_records_extended (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    original_record_id TEXT NOT NULL, -- Cambiado a TEXT para compatibilidad
    clinic_id TEXT, -- Agregar clinic_id directamente para queries
...
CREATE INDEX IF NOT EXISTS idx_medical_extended_original_record ON medical_records_extended(original_record_id);

-- Tabla 2: clinic_metrics (CORREGIDA)
CREATE TABLE IF NOT EXISTS clinic_metrics (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    clinic_id TEXT NOT NULL, -- Cambiado a TEXT para compatibilidad
...
-- Tabla 3: clinic_alerts (CORREGIDA)
...
-- Tabla 4: inventory_items (CORREGIDA)
...
-- Tabla 5: lab_tests (CORREGIDA)
CREATE TABLE IF NOT EXISTS lab_tests (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    clinic_id TEXT NOT NULL, -- Cambiado a TEXT para compatibilidad
    patient_id TEXT NOT NULL, -- Cambiado a TEXT para compatibilidad
    ...
    requesting_vet_id TEXT,
    processing_tech_id TEXT,
...
CREATE TABLE IF NOT EXISTS surgeries (
    ...
    -- Staff (ajustados a TEXT)
    surgeon_id TEXT,
    assistant_ids TEXT[], -- Array de assistant vet IDs
...
-- Verificar estructura de las nuevas tablas
SELECT 
    table_name,
    column_name,
    data_type,
...
/*
-- Primero verificar que existan clínicas
SELECT id, name FROM clinics LIMIT 1;

-- Insertar métricas de prueba (solo si hay clínicas)
...
-- 1. Tipos de datos de las tablas existentes
-- 2. Mensajes de confirmación de creación de tablas
-- 3. Lista final de tablas creadas
-- 4. Estructura completa de las nuevas tablas
